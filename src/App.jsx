...
export default function Spark() {
  ...
  useEffect(() => {
    if (step === "quiz" && ta.current)
      setTimeout(() => ta.current?.focus(), 300);
  }, [step, cur]);
  ...
  function back() {
    ...
    setDraft(ans[questions[cur-1].id]?.answer || "");
    setCur(c => c-1);
  }
  ...
  async function go(all) {
    ...
    const prompt = [
      "You are a sharp product strategist. Someone answered 8 questions about daily frustrations.",
      ...
      "Give 3-5 problems and 5-7 ideas tailored to THEIR answers. Start with { end with }",
      "",
      lines,
    ].join("\n");
  ...
  const R = { minHeight:"100dvh", background:"linear-gradient(160deg,#0a0a0f 0%,#0f0f1e 60%,#0a0a0f 100%)", fontFamily:"'DM Sans',system-ui,sans-serif", color:"#e8e6f0", overflowX:"hidden" };
  const G = `
    ...
    <div style={{...R,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"32px 20px 48px"}}>
      ...
      <div style={{maxWidth:400,width:"100%",animation:"fU .6s ease both"}}>
        <div style={{textAlign:"center",marginBottom:28}}>
          <div style={{display:"inline-flex",alignItems:"center",justifyContent:"center",width:80,height:80,borderRadius:24,background:"linear-gradient(135deg,#1a1008,#0f0a1a)",border:"1px solid rgba(250,204,21,0.2)",boxShadow:"0 0 40px rgba(249,115,22,0.2)",marginBottom:20}}>
            <svg width="42" height="42" viewBox="0 0 96 96" fill="none">
              <defs>
                <linearGradient id="isp" x1="48" y1="12" x2="48" y2="84" gradientUnits="userSpaceOnUse"><stop offset="0%" stopColor="#FACC15"/><stop offset="55%" stopColor="#F97316"/><stop offset="100%" stopColor="#EF4444"/></linearGradient>
        ...
        <div style={{display:"flex",flexWrap:"wrap",gap:8,justifyContent:"center",marginBottom:36}}>
          {["8 questions","AI-powered","~3 minutes","Free"].map(t=>(
            <span key={t} style={{fontSize:12,padding:"5px 12px",borderRadius:100,background:"rgba(255,255,255,0.05)",border:"1px solid rgba(255,255,255,0.08)",color:"#64748b"}}>{t}</span>
          ))}
        </div>
        <button onClick={()=>setStep("quiz")} style={{width:"100%",padding:"17px",background:"linear-gradient(135deg,#6366f1,#8b5cf6)",border:"none",borderRadius:16,color:"#fff",fontFamily:"'Syne',sans-serif",fontSize:17,fontWeight:700,cursor:"pointer",boxShadow:"0 8px 32px rgba(99,102,241,0.4)"}}>
          Let's go \u26A1
        </button>
      ...
      <div style={{flex:1,overflowY:"auto",padding:"20px 20px 0"}}>
        <div className="qc">
          <div style={{display:"inline-flex",alignItems:"center",gap:6,padding:"5px 12px",borderRadius:100,background:(CC[q.category]||"#888")+"20",border:"1px solid "+(CC[q.category]||"#888")+"40",marginBottom:16}}>
            ...
            <span style={{fontSize:11,fontWeight:600,letterSpacing:"0.08em",color:CC[q.category]}}>{q.category}</span>
          </div>
          <p style={{fontSize:20,fontWeight:600,lineHeight:1.4,color:"#f1f5f9",marginBottom:20,letterSpacing:"-0.01em"}}>{q.text}</p>
          <div style={{position:"relative"}}>
            <textarea ref={ta} value={draft} onChange={e=>setDraft(e.target.value)}
          ...
          {err&&<div style={{marginTop:14,padding:"12px 14px",background:"rgba(239,68,68,0.08)",border:"1px solid rgba(239,68,68,0.2)",borderRadius:10}}>
            <div style={{color:"#f87171",fontSize:12,fontWeight:600}}>\u274C {err}</div>
          </div>}
          <div style={{display:"flex",gap:5,justifyContent:"center",marginTop:24}}>
            {questions.map((_,i)=>(
              <div key={i} style={{width:i===cur?18:6,height:6,borderRadius:3,background:i<cur?"#6366f1":i===cur?"#8b5cf6":"rgba(255,255,255,0.1)",transition:"all .3s"}}/>
            ))}
          </div>
        </div>
      </div>
  ...
  if (step==="loading") return (
    <div style={{...R,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:24,gap:16}}>
      ...
      <div style={{textAlign:"center"}}>
        ...
        <p style={{color:"#475569",fontSize:13}}>Finding problems and generating ideas</p>
      </div>
      <div style={{display:"flex",flexDirection:"column",gap:8,marginTop:16}}>
        {["Identifying patterns\u2026","Mapping to real problems\u2026","Generating your ideas\u2026"].map((t,i)=>(
          <div key={i} style={{fontSize:12,color:"#334155",animation:"pl 2s ease infinite",animationDelay:i*.6+"s",textAlign:"center"}}>{t}</div>
        ))}
      </div>
    </div>
  );
  ...
  if (step==="results"&&res) return (
    <div style={{...R,minHeight:"100dvh"}}>
      ...
      <div style={{padding:"20px 16px 48px"}}>
              ...
              <div className="ri" key={i} style={{background:"rgba(255,255,255,0.03)",border:"1px solid rgba(255,255,255,0.08)",borderRadius:16,padding:"16px",animationDelay:(i*.08+.2)+"s"}}>
                ...
                {(idea.solves||[]).length>0&&(
                  <div style={{display:"flex",flexWrap:"wrap",gap:6}}>
                    {(idea.solves||[]).map((s,j)=>(
                      <span key={j} style={{fontSize:10,padding:"3px 9px",borderRadius:100,background:"rgba(139,92,246,0.12)",color:"#a78bfa",border:"1px solid rgba(139,92,246,0.2)"}}>Solves: {s}</span>
                    ))}
                  </div>
                )}
              </div>
        ...
        <button onClick={()=>{setStep("intro");setCur(0);setAns({});setDraft("");setRes(null);setErr(null);}}
